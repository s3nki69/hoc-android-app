...
class ArticlesFragment : Fragment() {
    ...
    private fun setupSwipeRefresh() { swipeRefresh.setOnRefreshListener { loadArticles() } }

    private fun filterArticles(query: String) {
        filteredArticles.clear()
        if (query.isEmpty()) { filteredArticles.addAll(articles) }
        else { filteredArticles.addAll(articles.filter { it.title.contains(query, ignoreCase = true) || it.excerpt.contains(query, ignoreCase = true) }) }
            ...
            try {
                ...
                for (i in 0 until jsonArray.length()) {
                    ...
                    try {
                        ...
                        // Felhő alapú (Jetpack) képek keresése
                        if (imageUrl.isNullOrEmpty() && post.has("jetpack_featured_media_url")) {
                            imageUrl = post.getString("jetpack_featured_media_url")
                        }
                    } catch (e: Exception) {}
                    
                    loadedArticles.add(Article(post.getInt("id"), title, excerpt, post.getString("link"), imageUrl, post.getString("date").substring(0, 10)))
                }
                withContext(Dispatchers.Main) {
                    articles.clear(); articles.addAll(loadedArticles); filterArticles(searchView.query.toString()); showContent()
                }
            } catch (e: Exception) { withContext(Dispatchers.Main) { showError("Hiba: ${e.message}") } }
        }
    }

    private fun openArticle(url: String) { startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(url))) }
